...
class CommandModeSwitchPlanner {
 public:
  static CommandModeSwitchPlanResult makePlan(const std::vector<ArmCommandModeState>& arm_states,
    ...
    for (std::size_t index = 0; index < arm_states.size(); ++index) {
      ...
      plan.arms.push_back(
          ArmCommandModeRequest{arm.arm_name, requested_mode, start.present || stop.present,
                                start.present ? initializationFor(start.mode)
                                              : (stop.present ? initializationFor(arm.current_mode)
 ...
 private:
  ...
  static bool addInterface(InterfaceLookup& interfaces,
                           std::string name,
  ...
  static CommandModeSwitchPlanResult buildInterfaceLookup(
      ...
      InterfaceLookup& interfaces) {
    if (arm_states.empty()) {
      return failure(CommandModeSwitchError::InvalidArmConfiguration,
                     "at least one configured arm is required");
    }
    ...
    for (std::size_t arm_index = 0; arm_index < arm_states.size(); ++arm_index) {
      ...
      if (!isSupportedCurrentMode(arm.current_mode)) {
        return failure(CommandModeSwitchError::UnsupportedCurrentMode,
      ...
      for (std::size_t joint = 1; joint <= 7; ++joint) {
        ...
        if (!addInterface(interfaces, prefix + "effort",
                          {arm_index, InterfaceSupport::JointTorque, joint_bit}) ||
            !addInterface(interfaces, prefix + "velocity",
                          {arm_index, InterfaceSupport::JointVelocity, joint_bit}) ||
            !addInterface(interfaces, prefix + "position",
      ...
      for (const auto* name : kCartesianPoseNames) {
        if (!addInterface(interfaces, arm.arm_name + "_ee_cartesian_position/" + name,
      ...
      for (const auto* name : kCartesianVelocityNames) {
        if (!addInterface(interfaces, arm.arm_name + "_ee_cartesian_velocity/" + name,
                          {arm_index, InterfaceSupport::Unsupported, 0})) {
          return failure(CommandModeSwitchError::InvalidArmConfiguration,
                         "arm names generate colliding command interface names");
        }
  ...
  static CommandModeSwitchPlanResult parseInterfaces(
      const std::vector<std::string>& requested_interfaces,
      const char* request_kind,
      const std::vector<ArmCommandModeState>& arm_states,
      const InterfaceLookup& interfaces,
    ...
    for (const auto& interface_name : requested_interfaces) {
      ...
      if (found == interfaces.end()) {
        if (claimsConfiguredArm(interface_name, arm_states)) {
          return failure(CommandModeSwitchError::UnknownInterface,
      ...
      if (description.support == InterfaceSupport::Unsupported) {
        return failure(CommandModeSwitchError::UnsupportedInterface,
                       std::string(request_kind) + " interface '" + interface_name +
      ...
      if (request.present && request.mode != mode) {
        return failure(CommandModeSwitchError::MixedJointModes,
  ...
  static bool claimsConfiguredArm(const std::string& interface_name,
                                  const std::vector<ArmCommandModeState>& arm_states) {
