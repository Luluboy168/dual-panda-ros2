...
class RobotSim {
 public:
  RobotSim() = delete;
  RobotSim(std::string& robot_name, bool has_gripper)
      : robot_name_(robot_name), has_gripper_(has_gripper) {}
  ~RobotSim();
 ...
 private:
  ...
  std::array<int, 9UL>
      joint_site_indices_;  // the joint sites positioned at the root acc. to the DH param
