...
bool RobotSim::populateIndices() {
  ...
  for (int n = 0; n < kNumberOfJoints; n++) {
    ...
    if (trq_id != -1) {
      ...
      std::cerr << "No torque mjOBJ_ACTUATOR " << trq_name << " found for " << joint_name
                << std::endl;
      return false;
    }
    if (vel_id != -1) {
      ...
      std::cerr << "No velocity mjOBJ_ACTUATOR " << vel_name << " found for " << joint_name
                << std::endl;
      return false;
    }
    if (pos_id != -1) {
      ...
      std::cerr << "No position mjOBJ_ACTUATOR " << pos_name << " found for " << joint_name
                << std::endl;
      return false;
    }
  }
  ...
  if (has_gripper_) {
    ...
    for (int n = 0; n < 2; n++) {
      std::string joint_name =
          robot_name_ + "_finger_joint" + std::to_string(n + 1);  // left, then right
      int joint_id = mj_name2id(m_, mjOBJ_JOINT, joint_name.c_str());
      if (joint_id != -1) {
...
franka::RobotState RobotSim::populateFrankaState() {
  ...
  */
  ...
  for (int i = 0; i < kNumberOfJoints; i++) {
    ...
    // the actual franka publishes non-zero values when in gravcomp mode, so add the qfrc_gravcomp
    // to match that
    current_state_.tau_J[i] =
        d->actuator_force[act_trq_indices_[i]] + d->qfrc_gravcomp[joint_qvel_indices_[i]];
...
bool RobotSim::setModelIndices() {
  franka_hardware_model_->setIndices(link_indices_, joint_site_indices_, joint_indices_,
                                     joint_qpos_indices_, joint_qvel_indices_, act_trq_indices_,
                                     act_pos_indices_, act_vel_indices_);
  return true;
}
