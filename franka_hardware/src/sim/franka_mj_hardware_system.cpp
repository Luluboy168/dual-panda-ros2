...
bool FrankaMjHardwareSystem::initSim(rclcpp_lifecycle::LifecycleNode::SharedPtr& model_nh,
  ...
  for (const auto& joint : info_.joints) {
    ...
    for (const auto& cmd_interface : joint.command_interfaces) {
      if (cmd_interface.name != hardware_interface::HW_IF_EFFORT &&    // Effort "effort"
          ...
          cmd_interface.name !=
              hardware_interface::HW_IF_VELOCITY  //&& // Joint velocity "velocity"
      ) {
    ...
    for (const auto& state_interface : joint.state_interfaces) {
      if (state_interface.name != hardware_interface::HW_IF_EFFORT &&  // Effort "effort"
          state_interface.name !=
              hardware_interface::HW_IF_POSITION &&  // Joint position "position"
          state_interface.name !=
              hardware_interface::HW_IF_VELOCITY  //&& // Joint velocity "velocity"
      ) {
        RCLCPP_FATAL(getLogger(), "Joint '%s' has unexpected command interface '%s'",
  ...
  for (size_t i = 1; i <= robot_count_; i++) {
    ...
    if (!arms_.insert(std::make_pair(mj_robot_name, ArmContainer())).second) {
      RCLCPP_FATAL(getLogger(),
                   "The provided robot namespace %s already exists! Make sure they are unique.",
                   mj_robot_name.c_str());
      return false;
    }
    ...
    bool has_gripper =
        info_.hardware_parameters.at("hand" + suffix) == std::string("True") ? true : false;
    ...
    if (arm.robot_->has_gripper_) {
      RCLCPP_INFO_STREAM(getLogger(),
                         "Gripper found for " << arm.robot_name_ << ". Creating a gripper server.");
      gripper_states_ptrs_.insert(
          std::make_pair(arm.robot_name_, std::make_shared<std::array<double, 3>>()));
      gripper_nodes_.insert(
          std::make_pair(arm.robot_name_, std::make_shared<franka_gripper::GripperSimActionServer>(
                                              rclcpp::NodeOptions(), arm.robot_name_)));
    ...
    for (size_t j = 7 * (i - 1); j < i * 7; j++) {
      ...
      d_->qpos[arm.robot_->joint_qpos_indices_[j - (7 * (i - 1))]] =
          std::stod(info_.joints[j].parameters.at("initial_position"));
      RCLCPP_DEBUG(getLogger(), "Joint %ld (name %s) initial position: %f", j,
  ...
  for (auto i = 0U; i < info_.joints.size(); i++) {
    ...
    state_interfaces.emplace_back(
        StateInterface(info_.joints[i].name, hardware_interface::HW_IF_VELOCITY,
                       &arms_[get_ns(info_.joints[i].name)].hw_velocities_.at(
...
hardware_interface::return_type FrankaMjHardwareSystem::read(const rclcpp::Time& /*time*/,
  ...
  for (auto& arm_container_pair : arms_) {
    ...
    if (arm.robot_->has_gripper_) {
      gripper_states_ptrs_[arm.robot_name_]->at(1) =
          d_->qpos[arm.robot_->gripper_joint_qpos_indices_[0]];
    }
  }
...
int main() {
    ...
    if (startsWith(str1, str2)) {
        ...
        std::cout << "The string \"" << str1 << "\" does not start with \"" << str2 << "\"." <<
std::endl;
    }
  ...
  */
  ...
  for (auto& arm : arms_) {
    ...
    for (auto& stop : stop_interfaces) {
      if (startsWith(stop, arm_ns)) {  // this basically has to check if the interface's first part
                                       // is basically "robot_name/", else ignore
    ...
    if (stop_type > 0) {
      ...
      is_duplicate =
          (is_effort && is_position) || (is_effort && is_velocity) || (is_velocity && is_position);
      if (!(is_effort || is_position || is_velocity)) {
        RCLCPP_ERROR(this->getLogger(),
                     "Requested stop interface is not a supported type!\n"
                     "Please make sure they are either effort, position or velocity.");
        return hardware_interface::return_type::ERROR;
      }
      if ((is_duplicate)) {
        RCLCPP_ERROR(this->getLogger(),
                     "Requested stop interface has a confusing name!\n"
    ...
    switch (stop_type) {
      case -1:
        RCLCPP_ERROR(this->getLogger(),
                     "Requested stop interfaces do not all have the same type!\n"
      ...
      case 1:  // stop the joint controllers
        ...
        if (arm_stop_interfaces.size() != kNumberOfJoints) {
          RCLCPP_ERROR(this->getLogger(),
                       "Requested joint stop interface's size is not 7 (got %ld)",
                       arm_stop_interfaces.size());
          return hardware_interface::return_type::ERROR;
        }
        for (size_t i = 0; i < kNumberOfJoints; i++) {
          if (is_effort) {
      ...
      case 2:  // Cartesian is not supported for simulation
        RCLCPP_ERROR(this->getLogger(),
                     "Cartesian interfaces are not supported for simulation!\n"
  ...
  for (auto& arm : arms_) {
    ...
    if (start_type > 0) {
      ...
      if (!(is_effort || is_position || is_velocity)) {
        RCLCPP_ERROR(this->getLogger(),
                     "Requested start interface is not a supported type!\n"
                     "Please make sure they are either effort, position or velocity.");
        return hardware_interface::return_type::ERROR;
      }
      if ((is_duplicate)) {
        RCLCPP_ERROR(this->getLogger(),
                     "Requested start interface has a confusing name!\n"
                     "Please make sure they are either effort, position or velocity, only.");
        return hardware_interface::return_type::ERROR;
      }
      ...
      if (arm.second.control_mode_ != ControlMode::None) {
        RCLCPP_ERROR(this->getLogger(),
                     "Switching between control modes without stopping it first is not supported.\n"
    ...
    switch (start_type) {
      ...
      case -1:
        RCLCPP_ERROR(this->getLogger(),
                     "Requested start interfaces do not all have the same type!\n"
                     "Please make sure they are either Cartesian or Joint.");
        return hardware_interface::return_type::ERROR;
      case 1:
        if (arm_start_interfaces.size() != kNumberOfJoints) {
          RCLCPP_ERROR(this->getLogger(),
                       "Requested joint start interface's size is not %ld (got %ld)",
                       kNumberOfJoints, arm_start_interfaces.size());
          return hardware_interface::return_type::ERROR;
        }
        if (is_effort) {
...
hardware_interface::return_type FrankaMjHardwareSystem::perform_command_mode_switch(
  ...
  for (auto& arm_p : arms_) {
    ...
    RCLCPP_INFO_STREAM(this->getLogger(),
                       "Arm " << arm.robot_name_ << " current mode: " << arm.control_mode_);
    if (arm.switch_cm_) {
