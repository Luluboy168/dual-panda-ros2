...
controller_interface::return_type MultiJointImpedanceExampleController::update(
  ...
  return all_commands_written ? controller_interface::return_type::OK
...
CallbackReturn MultiJointImpedanceExampleController::on_init() {
  try {
    ...
    if (num_robots <= 0) {
      RCLCPP_ERROR(get_node()->get_logger(), "arm_count must be greater than zero");
      return CallbackReturn::ERROR;
    }
    for (int arm = 1; arm <= num_robots; ++arm) {
      const auto prefix = "arm_" + std::to_string(arm) + ".";
...
CallbackReturn MultiJointImpedanceExampleController::on_configure(
  ...
  for (int i = 1; i <= num_robots; i++) {
    ...
    if (arm_id.empty()) {
      RCLCPP_ERROR(get_node()->get_logger(), "%sarm_id must not be empty", prefix.c_str());
      return CallbackReturn::FAILURE;
    }
    const auto inserted = arms_.emplace(arm_id, ArmContainer());
    if (!inserted.second) {
      RCLCPP_ERROR(get_node()->get_logger(), "Arm IDs must be unique: '%s'", arm_id.c_str());
      return CallbackReturn::FAILURE;
    }
