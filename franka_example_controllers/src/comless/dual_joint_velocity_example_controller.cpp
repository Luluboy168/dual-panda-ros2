...
#include "franka_example_controllers/comless/dual_joint_velocity_example_controller.hpp"
...
controller_interface::return_type DualJointVelocityExampleController::update(
  ...
  for (size_t arm_index = 0; arm_index < arms_.size(); ++arm_index) {
    for (int i = 0; i < num_joints; i++) {
      if (i > 2 && i < 7) {
        all_commands_written = command_interfaces_[k].set_value(omega) && all_commands_written;
      } else {
        all_commands_written = command_interfaces_[k].set_value(0.0) && all_commands_written;
      }
